...
class VideoProcessor
{
	...
	public function upload($videoUploadData)
	{
		...
		//uploads/videos/45kjlk45 playingdog.
	...
	private function processData($videoData,$filePath)
	{
		...
		{
			echo "The file is too large. Can't be more than  ".$this->sizeLimit." bytes";
			return false;
		}
	...
	{
		$query = $this->con->prepare("INSERT INTO videos(title,uploadedBy,discription,privacy,category,filePath) 
						VALUES(:title,:uploadedBy,:discription,:privacy,:category,:filePath)");
		$query->bindParam(":title",$uploadData->title);
		$query->bindParam(":uploadedBy",$uploadData->uploadedBy);
		$query->bindParam(":discription",$uploadData->description);
    ...
    public function generateThumbnails($filePath)
    {
    	...
    	for($num = 1; $num<=$numThumbnails;$num++)
    	{
    		...
    		$cmd = "$this->ffmpegPath -i $filePath -ss $interval -s $thumbnailsSize -vframes 1 $fullThumbnailPath  2>&1";
    ...
    private function updateDuration($duration,$videoId)
    {
    	...
    	$query = $this->con->prepare("UPDATE videos SET duration = :duration
    		where id = :videoId");
